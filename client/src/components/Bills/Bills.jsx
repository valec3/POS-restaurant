...
const Bills = () => {
    ...
    useEffect(()=>{
      setOrders([
        new OrderClass('2B',true,2), new OrderClass('3B',false,5),new OrderClass('2B',true,2), new OrderClass('3B',false,5),new OrderClass('2B',true,2), new OrderClass('3B',false,5),new OrderClass('2B',true,2), new OrderClass('3B',false,5),new OrderClass('2B',true,2), new OrderClass('3B',false,5),new OrderClass('2B',true,2), new OrderClass('3B',false,5),new OrderClass('2B',true,2), new OrderClass('3B',false,5),new OrderClass('2B',true,2), new OrderClass('3B',false,5),new OrderClass('2B',true,2), new OrderClass('3B',false,5),new OrderClass('2B',true,2), new OrderClass('3B',false,5),new OrderClass('2B',true,2), new OrderClass('3B',false,5),
      ])

    },[])

    const selectOrder = (id)=>{
      setCurrentTable(id +1)
      if(!OrDectail){
        setOrDectail(true)
      }else{
        setOrDectail(false)
    ...
    return (
        <div className='billBigBox'>
            <section className="billContainer">
                ...
                <p className="userInfo-name">Order # {currectTable}</p>
                </div>
                <header className="billContainer-header" >
                <p className="billContainer-title">Bills</p>
                <div className="billContainer-svgContainer">
                <svg onClick={()=>console.log('orders',orders)} className="billContainer-svg" width="64px" height="64px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M4 12H20M12 4V20" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                </div>
                </header>
                ...
                <section className="billCards">
                  {/* all orders  */}
                    {orders.map((items , index )=>(
                      <div onClick={()=>selectOrder(index)} className={orders == index?`orderContainerMap`:""} key={index}>
